...
class RecurrentLayerWithSearch_multi(Layer):
    """A copy of RecurrentLayer from groundhog"""
    ...
    def step_fprop(self,
        ...
        #          for the following code to make any sense.
            ...
            source_len = c[i].shape[0]
            source_num = c[i].shape[1]
            target_num = state_before.shape[0]
            dim = self.n_hids
...
class RecurrentLayerWithSearch_multiseperate(Layer):
    """A copy of RecurrentLayer from groundhog"""
...
class Decoder_joint(EncoderDecoderBase):
    ...
    def build_decoder(self, c, y,
        ...
        if self.state['dropout_encoder'] != 1.:
            dropout_encoder = self.trng.binomial((self.state['num_systems'],), n=1, p = self.state['dropout_encoder'], dtype="float32")
        else:
            dropout_encoder = None
        '''
        if not self.state['search']:
...
class SystemCombination(object):
    ...
    def __init__(self, state, rng,
        ...
        if not state.has_key('same_encoder'):
    ...
    def build(self):
        ...
        if not self.state['same_encoder']:
            ...
            for i in xrange(self.state['num_systems']):
                ...
                self.backward_sampling_c = self.backward_encoder.build_encoder(
                        self.sampling_x[i][::-1],
                        approx_embeddings=self.encoder.approx_embedder(self.sampling_x[i][::-1]),
                        return_hidden_layers=True).out[::-1]
                if self.state['forward']:
                    sampling_c_components.append(self.forward_sampling_c)
                if self.state['backward']:
