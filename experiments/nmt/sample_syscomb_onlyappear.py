...
class BeamSearch(object):
    ...
    def __init__(self, enc_dec):
        ...
        self.idx2word = cPickle.load(open(state['indx_word'], 'rb'))
        self.word2idx_trg = cPickle.load(open(state['word_indx_target'], 'rb'))
    ...
    def search(self, seq, n_samples, ignore_unk=False, minlen=1):
        ...
        x = []
        last_split = -1
        prob_mask = numpy.ones((self.eos_id+1), dtype="float32")*numpy.inf
        prob_mask[self.eos_id] = 1.
        #print self.word2idx_trg
        #print self.idx2word
        for i in xrange(len(seq)):
            if seq[i] == self.split_id:
                ...
                x.append(tmp)
                last_split = i
            elif seq[i] < self.source_eos_id:
                #print seq[i]
                #print self.idx2word[seq[i]]
                if self.word2idx_trg.has_key(self.idx2word[seq[i]]):
                    #print self.word2idx_trg[self.idx2word[seq[i]]]
                    prob_mask[self.word2idx_trg[self.idx2word[seq[i]]]] = 1.
        #print prob_mask
        ...
        '''
        print len(c)
        for i in c:prob_mask = numpy.zeros((,self.enc_dec.state))
            print i.shape
        '''
